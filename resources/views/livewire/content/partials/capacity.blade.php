<div x-show="$wire.viewMode === 'capacity'" class="space-y-6">
    ...
    {{-- 5-Step Action Panel for Over Capacity --}}
    @php
        ...
        <div class="border border-red-200 dark:border-red-800 rounded-lg p-5 bg-red-50 dark:bg-red-900/10 space-y-5">
            <div class="flex items-center justify-between">
                <h4 class="text-sm font-semibold text-red-700 dark:text-red-300">
                    ⚠️ Over Capacity — 5-Step Action Required
                </h4>
                @php
                    $allResolved = collect($overRows)->every(fn($r) => $r['confirmed_by']);
                @endphp
                @if($allResolved)
                ...
                <div class="border border-red-100 dark:border-red-800/50 rounded-lg p-4 bg-white dark:bg-red-950/20 space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-xs text-red-600 dark:text-red-400">
                                {{ number_format($row['total'] - $row['max'], 1) }}h over ({{ number_format($row['total'], 1) }}h / {{ number_format($row['max'], 1) }}h)
                            </span>
                        </div>
                        @if($row['confirmed_by'])
                    ...
                    @if(!$row['confirmed_by'])
                        ...
                        <div class="flex items-center gap-2 pt-2">
                            <flux:button size="sm" wire:click="saveCapacityResolution({{ $picId }})" variant="primary">
                                Simpan Tindakan
                            </flux:button>
                            <flux:button size="sm" wire:click="confirmCapacity({{ $picId }})" variant="outline">
                                ✓ Tandai Teratasi
                            </flux:button>
                        </div>
                    @endif
